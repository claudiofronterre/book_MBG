...
the AnPIT diagnostic plot in R, we can use the `assess_pp()` function in
RiskMap
...
using a clustering algorithm approach. We then use the `assess_pp()`, to
...
The `assess_pp()` function also allows the user to compute the CRPS and
...
Note that that `assess_pp()` can be run once in order to obtain both the
...
2.  Consider the example in @sec-liberia-anpit. Fit a third model, that
    ...
    and $M_1$. Make sure to apply the function `assess_pp()` to all three
