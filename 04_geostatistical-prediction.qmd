...
`pred_over_grid()` function in RiskMap, to sample from the predictive
distribution of the spatial Gaussian process $S(x)$ based on a regular
grid spanning the country of Liberia.

The `pred_over_grid()` function enables us to predict the separate effects
...
the `pred_over_grid()` function, we need to do 1) obtain a shape file that
...
`pred_over_grid()` function for both marginal and joint predictions, as
demonstrated below.
...
to the argument `contol_sim` of `pred_over_grid()`. The diagnostic on the
