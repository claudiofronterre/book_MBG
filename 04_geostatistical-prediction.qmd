...
scheme (`method = "clusterized"`), where the locations are at least 20
kilometers (`min_dist = 20`) apart from each other to create 10 test
sets (`iter = 10`) each of size 9 (`n_size = 9`) which corresponds to
...
SCRPS through the argument `which_metric` as shown below.
...
`which_metric = c("AnPIT", "CRSP", "SCRPS")` which computes all three
