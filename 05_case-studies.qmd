...
ghana <- readRDS("data/gha_adm0_geoboundaries.rds")
...
```{r}
#| label: prepare-malnutrition-data
# Load packages
library(RiskMap)
library(ggplot2)
library(dplyr)
...
```{r}
#| label: fit-malnutrition-mixed-models
library(lme4)
haz_lmer <- lmer(HAZ ~ age + pmax(age - 2, 0) + wealth + (1 | loc_id),
                 data = malnutrition)

summary(haz_lmer)

waz_lmer <- lmer(WAZ ~ age + pmax(age - 1, 0) + wealth + (1 | loc_id),
                 data = malnutrition)
summary(waz_lmer)
```
...
We next explore the residual spatial correlation through the empirical variogram. Hence, we extract the estimate of the random effects from @eq-model-haz-waz and input these into the `variogram()` function that generates the empirical variogram.
...
# Variogram for HAZ
var_haz <- variogram(re, "Z_hat_haz", distance_units = "km",
                       n_permutations = 1000)
var_waz <- variogram(re, "Z_hat_waz", distance_units = "km",
                       n_permutations = 1000)
library(patchwork)

# Create ggplot objects
p_haz <- plot_variogram(var_haz, plot_envelope = TRUE) + 
  ggtitle("HAZ")

p_waz <- plot_variogram(var_waz, plot_envelope = TRUE) + 
  ggtitle("WAZ")
...
```{r}
#| label: prepare-malnutrition-prediction-data
#| eval: false
...
```{r}
#| label: create-ghana-prediction-grid
#| eval: false
# Boundaries of Ghana
library(geobounds)
...
We then use the `predict_grid_target()` function to generate predictive summaries of stunting and underweight prevalence, as defined in @eq-prev-maln. To quantify uncertainty in the predictions, we report the coefficient of variation, which provides a standardized measure of relative uncertainty across the prediction grid.

```{r}
#| label: predict-malnutrition-prevalence
#| eval: false
...
Finally, we visualize the results in the map shown in @fig-maln-pred. The maps reveal a clear hotspot for both stunting and underweight, with predicted prevalence values reaching approximately $56\%$ and $27\%$, respectively. Notably, this same region also exhibits lower predictive uncertainty, measured by the coefficient of variation, compared to other areas in Ghana.
...
To assess the predictive performance and calibration of our geostatistical models for HAZ and WAZ, we perform a cross-validation exercise using the `assess_prediction()` function. Cross-validation is conducted by repeatedly splitting the data into training and testing sets, ensuring that prediction locations are at least 5 km away from the nearest training point. Specifically, we set `size = 100` prediction locations for the test set, impose a minimum distance constraint of 5 km (`min_dist = 5`), and repeat the process over 10 test sets randomly drawn under those conditions (`iter = 10`) using `method = "regularized"` as the sampling method.

```{r}
#| label: assess-malnutrition-calibration
#| eval: false
# HAZ
set.seed(123)
...
We then use geobounds to obtain Malawi’s administrative boundary and extract *P. falciparum* prevalence survey data from the malariaAtlas package for surveys conducted in or overlapping the year 2006.

```{r}
#| label: download-malawi-data
#| eval: false
# Get Malawi boundary
...
```{r}
#| label: download-malawi-covariates
#| eval: false
# -------------------------------
...
After defining the multi-band raster image `covariates` in Google Earth Engine, we download it as a GeoTIFF file using the `ee_as_rast()` function from the rgee package. The file is saved temporarily and clipped to the bounding box of Malawi. We then read the raster into R using the terra package.

```{r}
#| label: export-malawi-covariates
#| eval: false
# -------------------------------
...
```{r}
#| label: fit-malawi-geostatistical-models
#| eval: false
...
We then compare the predicted prevalence from each model using a regular 5 by 5 km grid covering the whole of Malawi, and display the results in @fig-pred-models. The maps show broadly similar spatial patterns, with both models identifying areas of high and low predicted prevalence in the southern region of the country. However, these visual comparisons do not allow us to determine which model performs better. We address this question in the next stage of the analysis by formally assessing the calibration and sharpness of the predictions generated by each model.

```{r}
#| label: predict-malawi-prevalence
#| eval: false
...
```{r}
#| label: prepare-mosquito-abundance-data
abund_sma <- abund_sma %>%
...
To provide spatial context, we use the geobounds package to download second-level administrative boundaries (ADM2) for the United States. We extract the boundaries for Placer, El Dorado and Sacramento counties which make up the SMA.

```{r}
#| label: load-sacramento-boundaries
#| include: false
sma_boundaries <- readRDS("data/sma_adm2_geoboundaries.rds")
```

```{r}
#| label: prepare-sacramento-boundaries
#| eval: false
library(geobounds)
...
```{r}
#| label: load-mosquito-abundance-model
#| include: false
glmer_wvn <- readRDS("data/glmer_wvn.rds")
```

```{r}
#| label: identify-mosquito-locations
#| eval: false
...
```{r}
#| label: summarize-mosquito-abundance-model
summary(glmer_wvn)
```
...
```{r}
#| label: extract-mosquito-random-effects
#| eval: false
...
#| fig-width: 10
#| fig-height: 15
plot_variogram(variogram_wnv, plot_envelope = TRUE)
```
...
Hence, we first simulate 1000 samples from the predictive distribution of $Z_i$ and denote those by $Z_i^{(j)}$, for $i=1,\ldots,1551$, and $j=1,\ldots,1000$.
```{r}
#| label: compute-mosquito-predictions
#| eval: false
n_samples <- 1000
set.seed(123)
...
```{r}
#| label: summarize-mosquito-predictions
#| eval: false
...
lambda_samples <- exp(
  matrix(eta0_ab, nrow = length(eta0_ab), ncol = n_samples) + samples_z)
...
```{r}
#| label: load-mosquito-predictions
#| echo: false
...
```{r}
#| label: compute-mosquito-anpit
#| eval: false
set.seed(123)
wnv_anpit_res <- anpit_wnv(glmer_wvn, test_prop = 0.30, nsim = 10000)
```
In the code above the AnPIT is computed using a test set corresponding to $30\%$ of the original dataset. The number of simulations used to approximate the AnPIT is set to 10000. Finally, we plot the results.
...
```{r}
#| label: load-wnv-infection-model
#| echo: false
...
```{r}
#| label: summarize-wnv-infection-model
summary(inf_fit)
```
...
```{r}
#| label: create-sacramento-prediction-grid
#| eval: false
...
```
```{r}
#| label: load-wnv-grid-predictions
#| echo: false
pred_inf_grid <- readRDS("data/pred_inf_grid.rds")
```

@fig-wnv-pred-map displays the predictive mean of $p(x)$ over the regular grid. The predicted prevalence of WNV ranges from approximately $1.3\%$ to $8.6\%$ across the study area, with the highest values concentrated in a small zone in the south-west.
...
Several U.S. programs consider a vector index (VI) of 0.75 or higher as indicative of likely human transmission and use this threshold to trigger intensified control measures [@cityofboulder_wnv_vectorindex_2025; @fortcollins_localdata_vi_2025; @fortcollins_wnv_program_manual_2014]. In @fig-vi, the dashed red line marks this operational threshold for elevated WNV risk. The predicted VI exceeds this threshold at locations in every year, although the number of exceedances declines after 2019. Under this operational criterion, the model therefore identifies periods and locations of elevated WNV risk throughout 2015--2021, with fewer exceedances in the final two years.


```{r}
#| label: load-vector-index-predictions
#| echo: false
...
```{r}
#| label: define-random-effect-simulation
#| eval: false
simulate_random_effects <- function(model, nsim = 1000, grid_range = c(-10, 10), grid_length = 5000) {
  # Extract fixed effects and RE variance
  beta_hat <- fixef(model)
...
```{r}
#| label: extract-anpit-model-matrix
#| eval: false
  D    <- model.matrix(model)
...
```{r}
#| label: simulate-anpit-random-effects
#| eval: false
  eta0 <- as.numeric(D %*% beta) + off
  u_draws <- rnorm(nsim, mean = 0, sd = sigma)
```
...
```{r}
#| label: extract-anpit-test-outcomes
#| eval: false
  y_test    <- y[test_idx]
  eta0_test <- eta0[test_idx]
  n_test    <- length(test_idx)
...
```{r}
#| label: compute-anpit-probabilities
#| eval: false
  for (i in seq_len(n_test)) {
    lam <- exp(eta0_test[i] + u_draws)
    Fi_y[i]      <- mean(ppois(y_test[i],    lambda = lam))
...
```{r}
#| label: average-nonrandomized-pit
#| eval: false
  npit_avg <- function(u, Fy, Fym1) {
    denom <- pmax(Fy - Fym1, .Machine$double.eps)
    lt <- u <= Fym1; gt <- u >= Fy; mid <- !(lt | gt)
