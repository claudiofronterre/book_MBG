...
waz_fit <- readRDS("data/waz_fit.rds")f
...
mean_nmosq <- readRDS("data/mean_nmosq.rds")
...
stunting_prev <-
predict_grid_target(haz_pred_grid,
...
underw_prev <-
  predict_grid_target(waz_pred_grid,
...
# HAZ
assess_haz <-
assess_prediction(list(HAZ = haz_fit),
          n_size = 100,
          min_dist = 5,
          iter = 10,
          method = "regularized")
...
# WAZ
assess_waz <-
  assess_prediction(list(WAZ = waz_fit),
            n_size = 100,
            min_dist = 5,
            iter = 10,
            method = "regularized")
...
#| eval: false
assess_pred_mlw <-
assess_prediction(list(all_cov = mod_all_cov,
               pca = mod_pca),
          method = "cluster",
...
```
In the code above the AnPIT is computed using a test-set corresponding to $30\%$ of the original dataset. The number of simulation used to approximate the AnPIT is set to 1000. Finally, we plot the results.
...
pred_S_inf <- setup_prediction(inf_fit,
                             grid_pred = grid_pred_sac)

pred_inf_grid <- predict_grid_target(pred_S_inf,
                                  f_target = list(prev = function(x) exp(x)/(1+exp(x))))
...
# abund_sma dataset
pred_S_loc <- setup_prediction(inf_fit, grid_pred = loc_pred)
