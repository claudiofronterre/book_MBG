...
for (i in seq_len(n_kappa)) {
  ...
  fit_galicia_list[[i]] <- glgpm(formula_i,
                                 data = galicia, family = "gaussian",
                                 distance_units = "km", messages = FALSE)
  llik_values[i] <- fit_galicia_list[[i]]$log_lik
...
## --- 6b. Same model, saved under the name used later in the
...
par_hat <- vector("list", n_sim)

for (i in seq_len(n_sim)) {
  sim_data <- simulated_data(liberia_boot, simulation = i)
